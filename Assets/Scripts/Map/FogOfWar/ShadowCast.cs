...
public static class ShadowCast
{
    public static void CalculateVisibility(Vector3 position, BoundsInt bounds, TileType[,] tiles, float[,] visibility)
    {
        ...
        //        visibility[x, y] = (y * visibility.GetLength(0) + x) % 2 == 0 ? 0.5f : 0f;
        //    }
        //}
        ...
        ComputeFieldOfViewWithShadowCasting(boundsPosition.x, boundsPosition.y, bounds.size.x / 2, (x, y) => tiles[bounds.x + x, bounds.y + y].HasFlag(TileType.Wall), (x, y) => visibility[x, y] = 1f);

        float[,] visibilityTarget = new float[visibility.GetLength(0), visibility.GetLength(1)];

        int kernelRadius = 3;
        int[] kernel = { 1, 6, 15, 20, 15, 6, 1 };
        for (int y = 0; y < visibility.GetLength(1); y++)
        {
            if (y < 0 || y >= tiles.GetLength(1))
            {
            ...
            for (int x = 0; x < visibility.GetLength(0); x++)
            {
                ...
                visibilityTarget[x, y] = 0f;
                int samples = 0;

                for (int xx = Mathf.Max(x - kernelRadius, 0); xx < Mathf.Min(x + kernelRadius, visibility.GetLength(0)); xx++)
                {
                    int k = kernel[xx + kernelRadius  - x];
                    visibilityTarget[x, y] += visibility[xx, y] * k;
                    samples += k;
                }
                visibilityTarget[x, y] /= samples;
        ...
        for (int y = 0; y < visibility.GetLength(1); y++)
        {
            ...
            for (int x = 0; x < visibility.GetLength(0); x++)
            {
                ...
                for (int yy = Mathf.Max(y - kernelRadius, 0); yy < Mathf.Min(y + kernelRadius, visibility.GetLength(0)); yy++)
                {
                    int k = kernel[yy + kernelRadius - y];
                    visibilityTarget[x, y] += visibility[x, yy] * k;
                    samples += k;
                }
                visibilityTarget[x, y] /= samples;
                visibility[x, y] = visibilityTarget[x, y];
        ...
        for (int y = 0; y < visibility.GetLength(1); y++)
        {
            ...
            for (int x = 0; x < visibility.GetLength(0); x++)
            {
                if (bounds.x + x < 0 || bounds.x + x >= tiles.GetLength(0))
                {
                    continue;
                }
